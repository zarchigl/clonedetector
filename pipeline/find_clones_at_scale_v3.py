"""
find_clones_at_scale_v3.py (Exclude LICENSE files from TF-IDF vectorization)
...
USAGE
-----
    python3 find_clones_at_scale_v3.py --datasets-dir datasets/mysql --threshold 80
    python3 find_clones_at_scale_v3.py --datasets-dir datasets_all --top-k 20
    python3 find_clones_at_scale_v3.py --datasets-dir datasets/mysql --method tfidf --threshold 66.6 --top-k 488
    python3 find_clones_at_scale_v3.py --datasets-dir datasets/mysql --method bert --threshold 88 --top-k 488
"""
...
from tfidf_similarity_v3 import read_role_as_text
from content_template import template_lines, strip_template
from bert_similarity_v3 import chunk_text, DEFAULT_MODEL


def load_roles(root_dir: Path, strip_content: bool = False) -> dict:
    """Returns {role_name: concatenated_text} for every immediate
    subfolder under root_dir with readable content.

    strip_content removes `ansible-galaxy init` TEMPLATE TEXT before the text is
    returned -- the content counterpart to filter_boilerplate(), which strips
    generated PATHS from structure similarity. OFF by default: turning it on
    changes every content score, so the tau you gate with must have been derived
    with the same setting (see content_template.py for the measured effect).
    """
    lines = template_lines(verbose=True) if strip_content else None
    roles = {}
    for entry in sorted(root_dir.iterdir()):
        if entry.is_dir():
            text = read_role_as_text(entry)
            if strip_content:
                text = strip_template(text, entry.name.split(".", 1)[-1], lines)
            if text.strip():
...
def embed_roles_tfidf(roles: dict, n_components: int) -> tuple:
    ...
    identical result to tfidf_similarity_v3.py's uncompressed vectors -- but
...
def find_candidate_pairs(role_names, matrix, index, top_k, threshold, role_paths=None,
                         threshold_on="content"):
    """
    ...
    the CONTENT similarity threshold. Returns deduplicated
    (role_a, role_b, content_pct, structure_pct, overall_pct) tuples with
    role_a < role_b alphabetically, so A-B and B-A collapse into one row
    instead of two.

    NOTE on what the threshold applies to: threshold_on selects the column
    the cutoff is checked against -- "content" (default, the FAISS-retrieved
    score) or "overall" (the mean of content and structure, Pandu's
    S_overall). Whichever you pick, the tau MUST have been validated for
    that same column; they are different score distributions.

    RETRIEVAL IS ALWAYS BY CONTENT. FAISS has no way to search on
    structure, since structure isn't part of the embedded vector, so
    top_k nearest neighbours are always chosen by content similarity.
    threshold_on="overall" therefore filters the content-retrieved
    candidate set -- it cannot introduce a pair that content retrieval
    never surfaced. Keep top_k large enough that this is not the binding
    constraint. If role_paths is None, structure/overall are left as
    None / equal to content (no structure signal available).
    """
    n = len(role_names)
    ...
    for i in range(n):
        for sim, j in zip(similarities[i], indices[i]):
            ...
            # Cheap early exit: when gating on content, a pair below tau can
            # never qualify, so skip the structure computation entirely.
            if threshold_on == "content" and content_pct < threshold:
                continue
...
def main():
    ...
    ap.add_argument("--filter-content-boilerplate", action="store_true",
                    help="Also remove `ansible-galaxy init` TEMPLATE TEXT from the TF-IDF "
                         "content (the content counterpart to --filter-boilerplate, which "
                         "only affects structure). MUST match the setting used to derive "
                         "tau, or the threshold will not apply to these scores.")
    ap.add_argument("--filter-boilerplate", action="store_true",
                     help="Exclude boilerplate paths from structure similarity. Fixes "
                          "unrelated roles scoring 80-100%% structure purely because both "
                          "were created by `ansible-galaxy init`. The tau MUST be re-derived "
                          "with the same settings on evaluate_scale_pipeline_accuracy_v3.py.")
    ap.add_argument("--boilerplate-mode", choices=list(BOILERPLATE_MODES), default="frequency",
                     help="How boilerplate is defined when --filter-boilerplate is set. "
                          "'galaxy_ci' is recommended for new technologies: it is a fixed list "
                          "needing no corpus and no tuning. 'frequency' is corpus-derived and "
    ...
    ap.add_argument("--threshold-on", choices=["content", "overall"], default="content",
                     help="Which score the --threshold cutoff applies to. MUST match the "
                          "column the tau was validated against in Step 1.")
    ...
    if args.threshold_on == "overall" and role_paths is None:
        ap.error("--threshold-on overall needs structure similarity; drop --no-structure.")
    pairs = find_candidate_pairs(role_names, matrix, index, args.top_k, args.threshold,
                                 role_paths, args.threshold_on)
    print(f"Found {len(pairs)} pairs at or above {args.threshold}% CONTENT similarity")
