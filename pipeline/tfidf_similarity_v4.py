"""
tfidf_similarity_v4.py (No EXCLUDE_FILENAMES, no LICENSE blocklist)
...
USAGE
-----
    python tfidf_similarity_v4.py --tech nginx --mode baseline
    python tfidf_similarity_v4.py --tech nginx --mode allpairs
    python tfidf_similarity_v4.py --all --mode baseline
...
# Reverted to include ALL files, no exclusions -- matching Pandu's
# original methodology, which does not exclude any file type from its
# git-diff-based content similarity either. Kept as an empty set (rather
# than removing the check entirely) so a future, evidence-based exclusion
# can be re-added in one place if needed.
